...
static const char* DAILY_HEADER =
...
void logDaily(const char* dateStr, uint32_t entries, uint32_t exits,
  ...
  f.printf("%s,%u,%u,%ld,%s,%s,%s\n", dateStr, entries, exits,
           (long)((int32_t)entries - (int32_t)exits),
           openingTime, closingTime, notes);
  f.close();
}
...
void saveDay(const DayCounters& day) {
  ...
  f.printf(
      ...
      "\"opening\":\"%s\",\"closing\":\"%s\"}\n",
      day.date, day.entries, day.exits, day.openingTime, day.closingTime);
  f.close();
}
